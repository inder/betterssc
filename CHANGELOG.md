...
- **One Telegram alert per trade message as it lands**, on the existing ✈ bridge and independent of the chat mirror: `🔴 SELL·closed QQQ — Jordan Kerner: close $QQQ 718 lottos` plus a **Link** that opens the thread scrolled to that comment (Substack's own `target_reply_id` permalink form, verified live). Alerts fire only for messages that arrive while the tab is open — never for history, backfill or a reload — and never twice for the same trade (the day's alert keys persist). Own messages alert too. A ticker guessed from a lowercase word is sent marked "unconfirmed" rather than dropped.
- **The parser is corpus-calibrated, not guessed.** `lib/trades.js` was built against 6,133 real comments from the live channel; the committed fixture (`test/fixtures/trade-messages.json`, 247 rows) keeps real positives AND real natural negatives from the same set, the author's root posts, hand-built adversarials across 12 failure modes (hypothetical, intent, question, verb-side and ticker-side idioms, quoted parent, negation, third-party attribution, past-tense narration, opposing verbs in one message, shouting, tickerless), terse valids, formatting rows, and 12 ET-day rows including both DST edges. Every row is asserted by a meta-test; every recall miss the design accepts is pinned so it can't be "fixed" back open by accident.
...
### Fixed — Reactions on already-loaded messages never showed up without a reload
- **Someone reacting to your message (or any older message) was invisible until you refreshed the tab.** The message poll (`pollNewMessages`) fetches `fetchCommentsAfter(postUuid, sinceISO)`, filtered by `created_at` — a reaction doesn't change a message's `created_at`, so it never arrived there. New `pollReactionRefresh()` runs on its own slower 20s timer, re-fetching the most recent page of the thread (`fetchCommentsBefore`, already used elsewhere for history pagination) and merging in any reaction-count change for a message already loaded — surgically patching just that message's DOM, no full re-render. It deliberately skips any comment id not already known, so new-message detection stays the main poll's exclusive job.
- **Independent review** (Fable, 0 Critical / 3 should-fix / 5 nit, all folded in before push) caught a real race in the first cut: the refresh poll's wholesale object-replace could revert a reaction *this tab itself* just sent, in the gap between the refresh fetch being issued and Substack's server registering the send — self-correcting a tick later, but a confusing flicker. Fixed by tracking in-flight *and* just-settled sends per comment id and comparing against the refresh fetch's issue time, not its processing time. Also caught: the refresh poll could wholesale-replace a still-`_pending`/`_failed` optimistic row, bypassing `reconcilePending`'s carry-forward logic (a recurrence of a previously-banked lesson) — now guarded; and `sendReaction` reacted against whatever object reference a caller happened to be holding rather than the live store entry, which the new poll's routine replacement made a real (if rare) way to silently lose your own reaction — now always resolves the live entry first.
- New pure helpers `normalizeReactionCounts` / `reactionsChanged` in `lib/compose.js` (handles both Substack's REST-numeric and WS-object reaction shapes). 9 new tests, 743/743 passing.

### Added — ✈ Telegram bridge: optional pinned-members-only filter
- **The Telegram mirror can now be scoped to just your pinned people.** A new checkbox in the bridge dialog ("Only forward messages from pinned members") reuses the existing pinned-member rail (`state.pinnedUserIds`, the same pin toggle already in the member list) as the filter set — no new concept to learn. Off by default, so the mirror keeps forwarding the whole feed unless you opt in.
- Self is pinned by default, so turning the filter on never hides your own messages — it only trims everyone else down to the people you've pinned.
- `shouldForward` (`lib/telegram.js`) takes the filter as an optional third argument and fails open (forwards) on a comment with no author id, so a malformed/attachment-only row is never silently swallowed by the filter. 5 new tests; 734/734 passing.

### Fixed — Google AI: `gemini-2.5-flash` retired (404), then its replacement timed out
- **Every Google-provider call failed with `google 404: This model models/gemini-2.5-flash is no longer available to new users. Please update your code to use models/gemini-3.6-flash`.** The default Google model is now `gemini-3.6-flash` (the replacement Google's own error names; `gemini-3.7`/`3.8-flash` also exist at the same price but the migration target is the like-for-like fast tier). The fix has to reach further than the default: the chosen model is persisted in `chrome.storage` and replayed verbatim into every request, so an already-saved `gemini-2.5-flash` would keep 404ing forever. New `resolveModelId` maps retired ids to their replacement both when the preference loads and inside `google.buildRequest`, so no re-selection in the Tune dialog is needed. Catalog pricing updated from Google's pricing page ($0.75 / $3.75 per 1M through 2026-12-31, stepping up 2027-01-01 — noted in code); the `gemini-2.5-pro` "~17× cost" label was stale against that and now reads ~2×. README cost table recomputed. 6 new tests.
- **Follow-up: the migrated model then timed out** (`google network error: signal timed out` on a 187-message Insights run). Gemini 3.x defaults to its HIGH (dynamic) thinking level, which on a 60K-char summary ran past the 30s Insights timeout. Requests to any 3.x+ Google model now set `generationConfig.thinkingConfig.thinkingLevel: "LOW"` (enum verified against the live v1beta discovery document; 2.x models never get the field, since the API errors on it). The Insights timeout goes 30s → 60s to match Ask. `google.parseResponse` now joins every non-`thought` text part instead of trusting `parts[0]`, and a `MAX_TOKENS` finish with no visible text says the cap was spent on thinking rather than the generic "no response text". The AI-moderation send path (fail-closed) gets its output cap raised 512 → 2048, because on Gemini 3.x thought tokens bill against that cap and a cap spent entirely on thinking would have BLOCKED the send as a review failure. **Independent code review** (Opus; 0 Critical, 5 should-fix, 5 nit — all folded in before push) caught the moderation-cap interaction, a "raise the cap in the dialog" error string that reached the moderation surface where no such control exists, and an untrimmed part-join. Still open: `gemini-2.5-pro` thinks by default with no off switch and gets only the longer timeout (noted in code). Not live-verified against Google's API from this session — first real Insights run is the verification. 11 more tests.
...
- **The fix is URL-level, not a rewrite.** Probing the live API from a logged-in tab confirmed every endpoint BetterSSC already depends on is intact and unchanged in shape — including `/community/posts/<uuid>/comments`, which still serves the replies inside a channel post. So nothing downstream of picking a post needed migrating.
- New `lib/chat-url.js` parses **both** URL generations (the legacy form still arrives from bookmarks and history) and builds the deep link back into Substack, now preferring the channel form so "open in Substack" stops relying on a redirect. Its `substack.com` host check is kept and tested adversarially — the parser runs on the URL of whatever tab the toolbar button was clicked from, and its output feeds API paths.
- A channel id **alone** is now enough to boot: `lib/api.js` gains `fetchChatChannel` / `fetchChannelPosts` for the two new `/api/v1/chat/channels/…` endpoints, and the app resolves the publication id from the channel record and a default post from the channel feed before anything reads either.
- **Which post it opens matters.** The newest post in a channel is usually a broadcast link with zero replies; the live conversation is an older thread. `pickLiveliestPost` ranks by newest *reply*, not newest post — ranking by `created_at` would have opened an empty stream, which reads as "BetterSSC loaded nothing", the exact symptom being fixed.
- Endpoint semantics were **captured live, not inferred** (the lesson from the v0.1 WebSocket chase): the channel post feed is descending by `created_at`, `order=asc` is accepted and ignored, `after=` is accepted and ignored, and only `before=<ISO>` actually pages. That's documented in `lib/api.js` next to the wrappers.
- 25 new tests (`test/chat-url.test.mjs`), including 10 adversarial URL rejections and the zero-reply-broadcast-post case. 651 tests passing.
- **Independent code review** (0 Critical, 5 should-fix, 5 nits — all folded in before commit) caught, among others: the resolution being async meant the composer was typeable for ~1-3s with both ids still null, so the resolve now runs first thing in `init()`, before any listener is bound; a channel record missing `publication_id` used to return *normally* and surface much later as "Publication null" and an opaque upload 404, and now throws; and the picker's timestamp field names were pinned only by hand-built fixtures, so a redacted **real captured response** is now committed (`test/fixtures/channel-posts-page1.json`) and asserted against — if Substack renames those fields the suite fails instead of silently opening an empty thread. The review also corrected the fix itself: deferring the app's un-hide until after resolution would have left the composer permanently unmounted, since its mount gate reads `classList` synchronously exactly once.
- **Still open:** posting a *new top-level* message to a channel is not yet wired — that write path hasn't been captured, and this project's rule is to decode a protocol before implementing it rather than guess. Replying inside a thread, which is what this client actually does, is unaffected.

## [0.10.0] — 2026-08-08

### Added — 🛡️ AI moderation before posting (bring your own key)
- **Review your own message before it posts.** Turn it on in Chat preferences and, before a send hits the channel, your configured AI provider reviews it for public-forum framing and can catch it before anyone sees it — never someone else's messages, only your own outgoing ones.
- **Catches a rapid-fire burst as one message.** Send 2-3 short follow-ups in quick succession (a "stream of consciousness") and, if none carries a mention/reply/attachment, they merge into a single reviewed message instead of posting — and reviewing — separately. The window is configurable (1s/2s/3s/5s, default 2s).
- **Three outcomes.** Clean: posts as typed. Blunt but not "offensive" (the model's default verdict for tone/framing problems): the composer shows the AI's reworded version and waits for you — edit it or press Send, it never auto-posts on a timer. Offensive, or a reply to a message the AI classifies as political: blocked, Send stays disabled until you actually edit the flagged text, your original message (and any attachment/reply target) stays right where you left it.
- **Rewording never touches a message with a mention, reply, or attachment** — those can only be blocked or sent verbatim, never rewritten, so an `@name` token can never be silently mangled. That's also the only path that can trigger the political-reply block, since it's the only one that ever carries a reply target.
- **Skip review** is a one-click settings toggle for once you trust the flow — bypasses AI review entirely and sends immediately, same as moderation being off.
- **Fails closed, always.** A network error, timeout, or an AI response BetterSSC can't parse never results in a silent unreviewed post — the message holds, you're told what happened, and your text is right there to resend or send unmoderated by turning the feature off.
- **Same BYOK key as AI Insights** (OpenAI / Anthropic / Google) — no separate setup. Uses the cheapest model on your configured provider and the last ~100 messages as background context; web search is deliberately off here for speed.
- Built as a 5-slice arc with independent design review before each slice and independent code review after — two design-review rounds on the review-call slice alone, after the first proposal turned out to conflict with the feature's own requirement that blunt (not just offensive) messages actually get reworded. Live dogfooding then caught three more real problems fixed in the same release: an auto-post countdown that didn't give a real chance to read the AI's suggestion (removed — manual confirm only, no timer); a silent multi-second gap with no indicator while a message sat waiting to be reviewed (closed — a continuous status notice now shows from the moment you hit send); and a reword that changed what was actually being said, not just how it was phrased (the prompt's "preserve meaning" instruction wasn't concrete enough — rewritten with a named example of exactly that failure and an explicit "don't invent a different claim" fallback).
- New `lib/moderation.js` (prompt builder + defensive fenced-JSON parser, since none of the three providers offer native JSON mode) — 29 tests, adversarial coverage across 11 distinct response-parsing failure modes. 626/626 tests passing.
