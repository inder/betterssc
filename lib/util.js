...
// Substack's channel feed is a list of posts, and most of them are broadcast
// link posts with zero replies; the live conversation is whichever post has
// the newest reply. Ranking by last reply time (falling back to the post's own
// created_at when it has none) reproduces the thread Substack's own client
// treats as active. Ranking by created_at alone would open today's link post
// with an empty message stream — which reads to the user as "BetterSSC loaded
// nothing", the exact failure this whole fix exists to clear.
export function pickLiveliestPost(threads) {
  ...
  for (const t of threads || []) {
    ...
    // so seed it at -Infinity rather than skipping it.
    const score = Number.isNaN(at) ? -Infinity : at;
    if (!best || score > bestAt) {
      best = cp;
      bestAt = score;
    }
  }
