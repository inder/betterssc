...
export function buildExplainSystemPrompt(contextString, opts = {}) {
  ...
  const webMode = opts.webSearchEnabled
    ...
    : "You do NOT have access to web search. Explain using only the thread below and general knowledge. If something is referenced that you genuinely cannot decode from the thread, say so briefly rather than guessing.";
  const imageMode = opts.hasImages
    ...
    : "";
  return `You are a seasoned professional trader explaining a SINGLE message from a private group chat to a reader who is catching up. Voice: sharp, plain-spoken desk-trader — no hedging filler, no "as an AI", no disclaimers. You're fluent in market structure, technical analysis, options flow, and macro, and you decode jargon for the reader instead of hiding behind it. Always separate what's actually being claimed from your read of it, and never bury the risk. ${lensHint}
...
export function buildExplainUserMessage(targetComment, opts = {}) {
  ...
  const linkBlock = links.length
    ? `\n\nLinks referenced in this message — read them with web search if they're relevant to the explanation:\n${links.map((u) => `- ${u}`).join("\n")}`
    : "";
