...
export const MODEL_CATALOG = {
  ...
  google: [
    // gemini-3.6-flash is priced $0.75 / $3.75 through 2026-12-31 and steps
    // up to $1.50 / $7.50 on 2027-01-01 (Google's pricing page) — bump
    // these then.
    { id: "gemini-3.6-flash", displayName: "gemini-3.6-flash (fast, cheap)", inputPer1M: 0.75, outputPer1M: 3.75 },
    { id: "gemini-2.5-pro", displayName: "gemini-2.5-pro (more capable, ~2x cost)", inputPer1M: 1.25, outputPer1M: 10.00 },
  ],
};

// Model ids a provider has retired. The user's chosen model is persisted in
// chrome.storage (bssc_ai_model) and replayed verbatim into every request,
// so a retired id keeps 404ing forever after the catalog moves on unless it
// is mapped at the boundary. Google cut gemini-2.5-flash off for new users
// in 2026-09 ("no longer available to new users ... use gemini-3.6-flash").
// Keys must never also be live catalog ids, or the alias would shadow a
// model the user can still legitimately pick.
export const LEGACY_MODEL_IDS = {
  "gemini-2.5-flash": "gemini-3.6-flash",
};

// Maps a retired model id to its replacement; anything else (including
// undefined) passes through unchanged so callers keep their own fallback.
export function resolveModelId(modelId) {
  if (typeof modelId !== "string") return modelId;
  return Object.prototype.hasOwnProperty.call(LEGACY_MODEL_IDS, modelId)
...
// Gemini 3.x defaults to its HIGH (dynamic) thinking level, which on a 60K-
// char summary blew past AI Insights' timeout ("google network error:
// signal timed out") the moment the default moved off 2.5-flash. LOW is
// plenty for summarise / answer / moderate. Enum values per the live v1beta
// discovery doc: MINIMAL | LOW | MEDIUM | HIGH. LOW rather than MINIMAL
// because LOW is the lowest level Google documents for EVERY 3.x model
// (MINIMAL is documented for the Flash line only, and neither has been
// exercised live against gemini-3.6-flash from this codebase), and the
// AI-moderation send path fails CLOSED on a 400 — an unaccepted enum value
// there would block every send, which is worse than a slower review.
export const GOOGLE_THINKING_LEVEL = "LOW";
// thinkingLevel is "recommended for Gemini 3 or later models; use with
// earlier models results in an error" (discovery doc) — so 2.x never gets
// it. Anything 3.x+ does. KNOWN GAP: gemini-2.5-pro (still in the catalog)
// also thinks by default and its budget can't be turned off; it gets no
// mitigation here beyond the longer Insights timeout. The 2.x-era field is
// thinkingConfig.thinkingBudget (min 128 on 2.5 Pro) — wire it only after a
// live call confirms the shape, per this project's decode-before-implement
// rule.
export function supportsThinkingLevel(modelId) {
  return typeof modelId === "string" && /^gemini-(?:[3-9]|\d{2,})(?:[.-]|$)/.test(modelId);
}
...
export const google = {
  ...
  buildRequest({ systemPrompt, conversation, apiKey, signal, model, maxTokens, webSearchEnabled, images }) {
    ...
    // Resolve retired ids here too (not only at storage load) so every
    // caller that hands us a model string gets the same protection.
    const modelId = resolveModelId(model) || google.model;
    const body = {
    ...
    if (supportsThinkingLevel(modelId)) {
      body.generationConfig.thinkingConfig = { thinkingLevel: GOOGLE_THINKING_LEVEL };
    }
    if (webSearchEnabled) {
      // Gemini google_search grounding (2.5+ tool shape). The response will carry a
  ...
  parseResponse(json) {
    ...
    // Thinking models can return several parts, and a part flagged
    // `thought: true` is reasoning, not the answer (only present when
    // includeThoughts is set, but never render it if it is). Join every
    // non-thought text part rather than trusting parts[0] to be the answer.
    const text = parts
      ...
      .trim();
    if (!text) {
      // Thought tokens are billed against maxOutputTokens, so a cap the
      // model spends entirely on reasoning comes back as MAX_TOKENS with
      // no visible text. Say so instead of the generic "no response text".
      // Deliberately no "raise the cap" instruction: this string reaches
      // the moderation send path verbatim, and that cap is hardcoded.
      if (cand && cand.finishReason === "MAX_TOKENS") {
        return { error: "Google: the output token cap was spent on thinking before any text was produced" };
      }
      return { error: "Google: no response text" };
    }
