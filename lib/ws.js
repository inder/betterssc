  ...
  async connect() {
    ...
    try {
      ...
      ws.addEventListener("open", () => {
        // The zyncrealtime protocol expects two consecutive {token} frames
        // on open: the first authenticates the session, the second triggers
        // subscriptions to all channels the JWT's `subscribe|*` perms cover.
        // Sending only one leaves the server waiting for the subscribe frame
        // and it eventually returns `{"error":"Invalid message"}` (~250ms).
        // Substack's own client also sends both frames immediately.
