  ...
  async connect() {
    ...
    try {
      ...
      ws.addEventListener("open", () => {
        ...
        console.log("[BetterSSC WS] → OPEN, sending auth frame 1/2");
        ws.send(frame);
        // v0.1.4: hold the second frame until after the auth OK arrives.
        // The server appears stateful — it expects an auth, processes it,
        // and only then is ready for the subscribe frame. Sending them
        // synchronously (v0.1.2) caused both to be tagged Invalid.
      ...
      ws.addEventListener("message", (event) => {
        ...
        // Auth-then-subscribe handshake — send the second token frame as
        // soon as the first OK ack arrives.
        if (!this._subscribeFrameSent) {
          try {
            ...
            if (
              ...
              const frame2 = JSON.stringify({ token: this.token });
              console.log("[BetterSSC WS] → sending subscribe frame 2/2");
              try {
                ws.send(frame2);
              } catch (_) {}
