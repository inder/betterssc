...
export const MAX_MESSAGE_LEN = 4096; // Telegram sendMessage hard limit
...
//   opts.pinnedOnly    : when true, restrict forwarding to pinned authors (the
//                         same pinned-member rail used elsewhere in the app —
//                         this is an OPTIONAL filter, off by default).
//   opts.isPinnedAuthor : (authorId) => boolean, required when pinnedOnly is
//                         true. A comment with no author id is forwarded
//                         regardless (fail open — never silently swallow an
//                         unattributed message because of a filter mismatch).
...
// from the mirror — the v0.9.0 "no messages from Substack" bug. pinnedOnly is a
// different, explicit opt-in filter — not the same self-skip mistake.)
export const shouldForward = (comment, sentIds, opts) => {
  if (!comment || comment.id == null) return false;
  if (comment._pending) return false;
  if (sentIds && sentIds.has(comment.id)) return false;
  if (opts && opts.pinnedOnly && typeof opts.isPinnedAuthor === "function") {
    const authorId = comment.author && comment.author.id;
    if (authorId != null && !opts.isPinnedAuthor(authorId)) return false;
  }
  return true;
};
