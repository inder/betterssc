...
// The id of the comment this one replies to (for native Telegram reply
// linkage), or null. Quote-first precedence (quote.id → quote_id → parent_id):
// the overlay's quote display + jump-to all key off c.quote.id (app.js:2226),
// so the quoted message is the most relevant reply target.
export const replyTargetId = (comment) => {
  if (!comment) return null;
  if (comment.quote && comment.quote.id != null) return comment.quote.id;
  if (comment.quote_id != null) return comment.quote_id;
  if (comment.parent_id != null) return comment.parent_id;
  return null;
};
...
// the bridge sets it false when it can instead use a NATIVE Telegram reply
// (reply_to_message_id), so the quote isn't shown twice.
...
// sendPhoto caption, capped at Telegram's 1024. opts.includeQuote as above.
