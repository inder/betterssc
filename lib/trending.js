...
// Exported for lib/trades.js (read-only consumer). A Set is mutable — never
// .add()/.delete() from outside, or trending's topic chips change silently.
export const TOPIC_STOPWORDS = new Set([
  ...
  // URL-scheme / host fragments (belt-and-suspenders — URLs are stripped
  // before topic extraction, but a bare "https"/"www" mention shouldn't trend)
...
// Strip whole URLs before topic extraction so a link never leaks its
// scheme/host fragments ("https", "www", host words like "tradingview") into
// the topic chips. Two forms: a scheme/www-prefixed URL, and a bare
// "host.tld[/path]" reference (e.g. "tradingview.com/chart"). The bare form
// requires an alphabetic TLD from a short allowlist so version strings
// ("v1.5") and abbreviations ("a.m.") aren't mistaken for domains.
// Tickers are extracted from the un-stripped text — a $TICKER or
// KNOWN_TICKERS symbol inside a URL is rare and still correctly a ticker.
const URL_RE =
  /\b(?:https?:\/\/|www\.)\S+|\b[a-z0-9-]+\.(?:com|net|org|io|co|ai|tv|news|finance|gov|edu)\b\S*/gi;
...
// Max symbols to chart from one search query — guards against an absurd
// all-caps query stacking dozens of heavy iframes.
const MAX_QUERY_CHART_TICKERS = 6;

/**
 * Given a raw search-box query, return the ordered, de-duplicated list of
 * stock/crypto symbols (UPPERCASE) the user is searching for — used to show
 * inline TradingView mini-charts above the search results. Pure + testable.
 *
 * Acceptance (mirrors the cashtag rules elsewhere, tuned for the search box
 * where the user types deliberately):
 *   - $SYMBOL → always charts (explicit cashtag, 1-6 letters, any case).
 *   - a bare token in KNOWN_TICKERS (any case) → always (e.g. typed "tsla").
 *   - a bare ALL-CAPS 3-5 letter token → only when EVERY token in the query
 *     is symbol-shaped ($-prefixed or all-caps 3-5). This charts a trending
 *     chip's bare symbol ("HPE", "DELL" — not in KNOWN_TICKERS) and an
 *     all-caps "HPE DELL" query, but NOT a stray capitalized word inside
 *     prose ("the HPE merger") or a lowercase word search ("great").
 * Author/command filters (@name, /from:, has:, since:) never chart.
 *
 * @param {string} raw  the search box value
 * @returns {string[]}  uppercase symbols, capped at MAX_QUERY_CHART_TICKERS
 */
export function extractQueryTickers(raw) {
  if (typeof raw !== "string") return [];
  const trimmed = raw.trim();
  // Author/command filters never chart: @name, /command, and bare key:value
  // commands (from:elon, has:link, since:3). The `\w+:` guard rejects the
  // slashless command forms explicitly rather than relying on them happening
  // to fail the symbol checks downstream.
  if (!trimmed || trimmed[0] === "@" || trimmed[0] === "/" || /^\w+:/.test(trimmed))
    return [];
  ...
  const parsed = tokens
    .map((t) => {
      const dollar = t[0] === "$";
      // Drop everything but letters (trailing punctuation, dots for class
      // shares — $BRK.B is out of scope for v1, consistent with the chat
      // extractor).
      const core = (dollar ? t.slice(1) : t).replace(/[^A-Za-z]/g, "");
      return { dollar, core };
    })
    .filter((p) => p.core);
  if (!parsed.length) return [];

  const isDollarSym = (p) => p.dollar && /^[A-Za-z]{1,6}$/.test(p.core);
  const isCapsShaped = (p) => !p.dollar && /^[A-Z]{3,5}$/.test(p.core);
  // "Symbol-only" query: every meaningful token looks like a symbol. Only
  // then do we trust a bare all-caps token that isn't in KNOWN_TICKERS.
  const allSymbolic = parsed.every((p) => isDollarSym(p) || isCapsShaped(p));

  const out = [];
  const seen = new Set();
  for (const p of parsed) {
    const upper = p.core.toUpperCase();
    let accept = false;
    if (isDollarSym(p)) accept = true;
    else if (KNOWN_TICKERS.has(upper)) accept = true;
    else if (allSymbolic && isCapsShaped(p)) accept = true;
    if (!accept || seen.has(upper)) continue;
    seen.add(upper);
    out.push(upper);
    if (out.length >= MAX_QUERY_CHART_TICKERS) break;
  }
  return out;
}
...
export function extractTrending(comments, opts = {}) {
  ...
  for (const c of comments || []) {
    ...
    // URLs are removed first so "https"/"www"/host words never trend.
    const topicText = clean.replace(URL_RE, " ");
    const seenWord = new Set();
    let wm;
    WORD_RE.lastIndex = 0;
    while ((wm = WORD_RE.exec(topicText))) {
      const raw = wm[0];
