 ...
 */
export function planTradeAlerts({ comments, now, pinnedOnly = false, pinnedIds, sentKeys, dayKey, notBefore } = {}) {
  const todayKey = etDateKey(now instanceof Date ? now : new Date()); // ONE clock read, hoisted
  // `notBefore`: the session's boot instant (minus a little slack). Substack's
  // initial page opens at the user's last-READ position, not the tail, so the
  // first poll after a reload can return a backlog of messages that arrived
  // while the tab was closed. Those show in the strip; they must NOT alert —
  // "alerts only for messages that arrive while the tab is open" (invariant 1).
  const floor = notBefore ? Date.parse(notBefore instanceof Date ? notBefore.toISOString() : String(notBefore)) : NaN;
  ...
  for (const c of comments || []) {
    ...
    if (Number.isFinite(floor)) {
      ...
      if (!Number.isFinite(t) || t < floor) continue; // pre-boot backlog: strip yes, alert no
    }
...
export function formatTradeAlert(message, { link } = {}) {
  ...
  // A short anchor, not the raw URL — the permalink is ~150 chars of uuids
  // and dominated the message in the first live run. href is attribute
  // text: entity-escape it and its quotes.
  const href = link ? escapeTelegramHtml(String(link)).replace(/"/g, "&quot;") : "";
  const linkLine = link ? `\n<a href="${href}">Link</a>` : "";
