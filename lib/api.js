...
const proxyUnreachableMessage = (e) =>
  `Couldn't reach your Substack tab — it may have navigated away ` +
  `from substack.com, been closed, or gone to sleep in the ` +
  `background. Open or refresh https://substack.com/chat in ` +
  `another tab and try again. (${(e && e.message) || e})`;

let cachedProxyTabId = null;

// `exclude` carries tab ids that already failed injection earlier in this
// same call — without it, a retry after a failed executeScript would just
// re-select the identical broken tab (tabs.query's ordering is stable),
// burning both retry attempts on the same dead end even when a second
// valid substack.com tab is open in another window.
const findProxyTab = async (exclude) => {
  const skip = exclude instanceof Set ? exclude : new Set(exclude || []);
  // Prefer the cached tab if it's still on substack.com (apex or sub) and
  // didn't just fail.
  if (cachedProxyTabId != null && !skip.has(cachedProxyTabId)) {
    try {
  ...
  // Prefer a tab we haven't already tried this call. If excluding leaves
  // nothing (e.g. the user has exactly one Substack tab open), fall back
  // to the full set — a lone tab still deserves its second attempt after
  // a transient failure (mid-navigation "frame was removed"-class errors
  // are real and often recoverable on immediate retry) instead of failing
  // outright with zero retries just because it's the only candidate.
  const candidates = tabs.filter((t) => !skip.has(t.id));
  const pool = candidates.length ? candidates : tabs;
  // Prefer a chat tab over any other substack.com tab.
  const chatTab = pool.find((t) => t.url && /\/chat\//.test(t.url));
  const tab = chatTab || pool[0];
...
const proxyFetch = async (path, init = {}) => {
  const tried = new Set();
  let tab = await findProxyTab(tried);
  let result;
  let attempts = 0;
  ...
  // invalidating the cache and re-querying we get a fresh tab, excluding
  // whichever tab id just failed so a second open substack.com tab (if
  // any) gets a chance instead of re-hitting the same dead one.
  for (;;) {
    attempts++;
    tried.add(tab.id);
    try {
      ...
      cachedProxyTabId = null;
      if (attempts >= 2) {
        throw new Error(proxyUnreachableMessage(e));
      }
      tab = await findProxyTab(tried);
    }
  }
  if (!result) {
    cachedProxyTabId = null;
...
// ---- Channel model (Substack chat migration, captured 2026-09-06) ----
//
// Publication chat is now a persistent CHANNEL holding many community
// posts, instead of a publication holding a flat post list. Two endpoints
// were captured live off substack.com/chat/group/<channelUuid>:
//
//   GET /api/v1/chat/channels/<channelUuid>
//     -> { channel: { id, publication_id, name, logo_url, paywall,
//                     post_permission, reply_permission, ... } }
//
//   GET /api/v1/chat/channels/<channelUuid>/posts
//     -> { threads: [{ communityPost, user, pubRole, ... }],
//          more, moreAfter, moreBefore, isPinned, lastViewedAt }
//
// The `communityPost` objects are the SAME shape the legacy endpoints
// return (they now also carry `channel_id`), and each post's replies are
// still served by the unchanged /api/v1/community/posts/<uuid>/comments —
// so everything downstream of post selection needs no migration.
//
// PAGINATION (verified against the live endpoint, not inferred): the posts
// list is DESCENDING by created_at regardless of an `order` param — passing
// order=asc returned the identical descending page — and only `before=<ISO>`
// actually pages. `after` was accepted and ignored. So: newest first, walk
// backwards with `before`, and trust `moreBefore` for "is there more".
//
// CAPTURE FACTS from the 2026-09-06 probe of a busy channel, recorded because
// the first-page-only assumption in resolveChannelTarget depends on them —
// and, since the thread rail (formatThreadRailRows) reads the same
// state.channelThreads, that dependency is now shared by TWO consumers, not
// one:
//   page size            25 threads
//   more / moreBefore    true / true (moreAfter false on the first page)
//   reply timestamps     all three of most_recent_comment_created_at,
//                        max_comment_created_at and created_at were PRESENT
//                        as keys on all 25 posts; the two reply fields were
//                        non-null on 13 of 25 and always equal to each other.
// Consequence to know about: since the page is ordered by created_at, not by
// last reply, a long-running discussion CAN be paged out by a run of newer
// broadcast posts — invisible to BOTH the picker (would pick a less-live
// default) and the rail (an older live thread just won't be listed). At
// 25/page against this channel's cadence the active thread was comfortably
// on page 1, so resolveChannelTarget only reads the first page — if that
// stops holding, page with `before` until a post with a non-null reply
// timestamp is found rather than widening the picker's or the rail's
// heuristics.
export const fetchChatChannel = (channelId) =>
  get(`/api/v1/chat/channels/${channelId}`);

export const fetchChannelPosts = (channelId, { before } = {}) => {
  const path = `/api/v1/chat/channels/${channelId}/posts`;
  if (!before) return get(path);
  const qs = new URLSearchParams({ before });
