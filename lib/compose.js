...
export const reconcilePending = (store, incoming) => {
  ...
  // Preserve the pending row's good author/avatar fields if the incoming
  // comment shape lacks them (Substack's POST response varies). Without
  // this, a reply could replace a "You" pending row with an authorless
  // comment that renders as "Unknown".
  const preserveAuthor = (incoming, pending) => {
    const out = { ...incoming };
  ...
  if (existing && existing._pending) {
    const merged = preserveAuthor(incoming, existing);
  ...
  for (const [pid, p] of store.comments) {
    ...
    const merged = preserveAuthor(incoming, p);
    store.comments.set(incoming.id, { ...merged, _pending: false });
    if (idx !== -1) {
...
// Build the extra POST-body fields for a reply. Returns `{}` when there's
// no replyingTo set, or `{parent_id, quote}` when there is. The shape of
// `quote` matches what Substack returns inside a reply payload — `comment`
// nested under it with the id of the original — so when our own reply
// echoes back, the existing renderMessageItem path already shows the
// quoted block correctly.
export const buildReplyFields = (composer) => {
  if (!composer || !composer.replyingTo || !composer.replyingTo.id) {
    return {};
  }
  const r = composer.replyingTo;
  return {
    parentId: r.id,
    quote: {
      comment: {
