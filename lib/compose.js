...
// Normalize a reactions object to a plain {name: count} map, zero-count
// entries dropped. REST shape is {name: <count number>}; the WS event shape
// observed is {name: {count, has_reacted}} — callers here only ever care
// whether COUNTS changed, not who reacted, so both reduce to the same shape.
export const normalizeReactionCounts = (reactions) => {
  const out = {};
  if (!reactions || typeof reactions !== "object") return out;
  for (const [name, v] of Object.entries(reactions)) {
...
// True when two reactions objects (either shape above) represent DIFFERENT
// reaction counts. Used by the reaction-refresh poll (app.js) to skip a DOM
// update when a re-fetched comment's reactions haven't actually moved.
export const reactionsChanged = (a, b) => {
  const an = normalizeReactionCounts(a);
  const bn = normalizeReactionCounts(b);
  const aKeys = Object.keys(an);
  if (aKeys.length !== Object.keys(bn).length) return true;
  for (const k of aKeys) {
    if (an[k] !== bn[k]) return true;
  }
  return false;
};
