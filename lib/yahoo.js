...
export async function fetchYahooQuotes(humanSymbols, proxyFn) {
  ...
  for (let i = 0; i < yahooSyms.length; i += BATCH) {
    ...
    // fetch() inside the substack.com tab's MAIN world. Earlier this
    // was a relative path, which fetch() resolved against substack.com
    // and silently returned the Substack homepage HTML; JSON.parse
    // failed in the catch below and chips stayed blank with no console
    // trace. Bug shipped in 60136ec, fixed here.
    const url =
    ...
    try {
      ...
      // Silent — single failed batch shouldn't kill the rest.
    }
  }
  return out;
}
