...
export function createTelegramBridge(deps = {}) {
  const {
    ...
    // Optional pinned-only filter (v0.12): (authorId) => boolean. Only
    // consulted when cfg.pinnedOnly is true; absent → filter never applies.
    isPinnedAuthor = null,
    // Test seam: the fetch used for Telegram API calls (defaults to the
    // global). Lets the queue be driven with fake timers and a fake API.
    fetchImpl = null,
  } = deps;
  const doFetch = fetchImpl || ((...args) => fetch(...args));

  const cfg = { token: null, chatId: null, streaming: false, pinnedOnly: false };
  ...
  async function tgCall(method, params) {
    ...
    const r = await doFetch(`${API}/bot${cfg.token}/${method}`, {
      method: "POST",
  ...
  function setConfig(next = {}) {
    ...
    if (next.pinnedOnly !== undefined) cfg.pinnedOnly = !!next.pinnedOnly;
  }
  function getConfig() {
    return {
      hasToken: !!cfg.token,
  ...
  // announce=true posts the session banner (the explicit user "Start streaming"
  // toggle). A tab-reload restore passes false so a reload doesn't re-banner.
  function enableStreaming(announce = false) {
    seedSent();
    cfg.streaming = true;
    if (announce) sendSessionBanner();
  }

  // Post a one-time "start of stream" banner when streaming is turned on (also
  // on a tab-reload restore — it marks the start of that session's stream).
  // Fire-and-forget and sent directly (not via the message queue) so it lands
  // before the first forwarded message; failure is non-fatal.
  function sendSessionBanner() {
    if (!cfg.token || cfg.chatId == null) return;
    let dateStr;
    try {
    ...
    tgCall("sendMessage", {
      chat_id: cfg.chatId,
      text: sessionBannerText(dateStr),
      parse_mode: "HTML",
      disable_web_page_preview: true,
    }).catch((e) => _log("session banner failed:", e && e.message));
  }
  ...
  function forwardNewMessages(comments) {
    ...
    for (const c of comments || []) {
      if (!shouldForward(c, sentIds, { pinnedOnly: cfg.pinnedOnly, isPinnedAuthor })) continue;
      sentIds.add(c.id); // mark before send so a re-poll can't double-enqueue
      sendQueue.push({ kind: "comment", comment: c });
      queued++;
    }
    if (comments && comments.length) {
      _log(`forward: ${comments.length} new, ${queued} queued for Telegram`);
    }
    drainSoon();
  }

  // Trade alerts (trade ticker arc, slice 3) ride the SAME queue as the
  // mirror so there is one rate limiter — but as a tagged item, never as a
  // bare comment: sendOne() runs pickImageUrl / formatMessageForTelegram /
  // rememberSent on whatever it gets, and a text payload through that path
  // would render as "<b>Unknown</b>" and index the inbound reply map by
  // undefined. Alerts do NOT require cfg.streaming — the mirror and the
  // alert are different products; only a connected bot (token + chat id).
  // Known limits, documented rather than handled here: a 429 is logged and
  // the item dropped (no retry_after honouring anywhere in this bridge),
  // and with the mirror ON a trade burst sends roughly twice as much.
  function enqueueText(payload) {
    if (!cfg.token || cfg.chatId == null) return false;
    if (!payload || typeof payload.text !== "string" || !payload.text) return false;
    sendQueue.push({ kind: "text", payload });
    drainSoon();
    return true;
  }
  ...
  async function sendText(payload) {
    ...
    // Deliberately no rememberSent(): an alert is not a mirrored comment, so
    // a Telegram reply to it must not route back as a Substack reply.
  }
  ...
  function drainSoon() {
    ...
    const tick = async () => {
      ...
      try {
        if (item && item.kind === "text") await sendText(item.payload);
        else await sendOne(item && item.comment);
      } catch (e) {
  ...
  async function sendOne(c) {
    // Always inline the quote as a <blockquote>. We deliberately do NOT use a
    // native Telegram reply (reply_to_message_id): the bot is the sender of
    // every forwarded message, so Telegram labels the reply preview with the
    // BOT's name ("betterssc"), losing the real quoted author. The inline
    // blockquote carries the correct author (it's our own <b>PKR</b> text).
    ...
    if (img) {
      try {
        const res = await tgCall("sendPhoto", {
          chat_id: cfg.chatId,
          photo: img,
          caption: formatPhotoCaption(c, { includeQuote: true }),
          parse_mode: "HTML",
        });
        rememberSent(res, c);
        return;
    ...
    const m = formatMessageForTelegram(c, { includeQuote: true });
    let text = m.text;
  ...
  return {
    ...
    _state: () => ({ sentCount: sentIds.size, queued: sendQueue.length, offset: updateOffset, msgMapSize: msgMap.size }),
  };
}
