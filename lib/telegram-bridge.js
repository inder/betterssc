...
export function createTelegramBridge(deps = {}) {
  ...
  // Reverse: Substack comment id → Telegram message_id, so a forwarded REPLY can
  // be linked to the quoted message via native reply_to_message_id (threading).
  const tgMsgByComment = new Map();
  ...
  function forwardNewMessages(comments) {
    ...
    for (const c of comments || []) {
      if (!shouldForward(c, sentIds)) continue;
  ...
  function rememberSent(result, c) {
    ...
    msgMap.set(mid, c.id);
    tgMsgByComment.set(c.id, mid);
    if (msgMap.size > MSG_MAP_MAX) {
      // drop oldest (Map preserves insertion order)
      const oldestMid = msgMap.keys().next().value;
      const oldestComment = msgMap.get(oldestMid);
      msgMap.delete(oldestMid);
      if (tgMsgByComment.get(oldestComment) === oldestMid)
        tgMsgByComment.delete(oldestComment);
    }
  }

  async function sendOne(c) {
    // Native reply linkage when we've already forwarded the quoted message;
    // otherwise inline the quote as a blockquote (so it shows either way, never
    // both). allow_sending_without_reply guards a since-deleted target.
    const target = replyTargetId(c);
    const replyTo = target != null ? tgMsgByComment.get(target) : undefined;
    const replyExtra =
      replyTo != null
        ? { reply_to_message_id: replyTo, allow_sending_without_reply: true }
        : {};
    const includeQuote = replyTo == null;
    ...
    if (img) {
      try {
        const res = await tgCall("sendPhoto", {
          chat_id: cfg.chatId,
          photo: img,
          caption: formatPhotoCaption(c, { includeQuote }),
          parse_mode: "HTML",
          ...replyExtra,
        });
        rememberSent(res, c);
    ...
    const m = formatMessageForTelegram(c, { includeQuote });
    let text = m.text;
