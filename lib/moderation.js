...
// 2048, not the 512 this was calibrated at: on Gemini 3.x thought tokens
// bill against maxOutputTokens even at the LOW thinking level, and a cap
// spent entirely on thinking comes back as MAX_TOKENS with no text — which
// this path treats as a review failure and BLOCKS the send. Output tokens
// cost only what the model actually emits, so the headroom is free unless
// used. Not a latency knob: a short verdict still stops when it stops.
export const MODERATION_MAX_TOKENS = 2048;
...
// Shown continuously from the moment a message is held/queued through
// review completion — the SAME string at both the hold-start call site
// and reviewAndSend's own call site, so re-showing it mid-flow is a
// visual no-op rather than a flicker. Was previously two different
// strings shown with a silent gap between them (nothing shown during the
// debounce wait) — confirmed by live dogfooding to read as "my message
// disappeared."
export const MODERATION_REVIEW_NOTICE = "Reviewing before it posts…";

// Shown once a reword is ready and waiting for the user to act on it.
// Deliberately NOT an auto-post countdown (a prior version auto-posted
// after a fixed delay — live dogfooding found that gave no real chance
// to read the suggestion before it went out) — the user must edit or
// explicitly press Send.
export const MODERATION_SUGGESTION_NOTICE =
  "Reworded for the channel. Review it, then press Send to post it (or edit it first).";
...
2. "needs_reword" — TRUE for a message that is blunt, divisive, inflammatory, or otherwise poorly-registered for a public forum, even though it is NOT "offensive". This is the DEFAULT verdict for tone/framing problems — most messages that need work will land here, not under "offensive". When needs_reword is true, also provide "reworded": a rewrite that changes ONLY tone and word choice, never the underlying claim. You are a copy editor here, not a fact-checker or a second author — you may NOT change WHAT is being asserted, only HOW it is phrased. This is the single easiest way to get this wrong: swapping a word for a "nicer" synonym can silently change the meaning, and that is worse than leaving the original tone alone. Concrete example of what NOT to do: the draft "Elon is pumping SpaceX stock" must NEVER become "Elon is buying SpaceX stock" — "pumping" (hyping/promoting to drive up the price, often implying manipulation) and "buying" (making a purchase) are different, unrelated claims; substituting one for the other puts words in the user's mouth that they never said. The correct move there is to keep the actual claim ("pumping/hyping the stock") and only soften surrounding tone if needed, or leave it as-is if the claim itself is what's blunt (bluntness alone is not a reason to reword — only actual public-forum-inappropriate FRAMING is). If you cannot find a rewording that preserves the exact claim, prefer needs_reword: false over inventing a different claim. Never soften a message into meaninglessness, and never add a claim the user didn't make. When needs_reword is false, set "reworded" to an empty string.
